...
function yulpToYul(source, t) {
  t.equal(print(compile(source).results), source, 'yulp to yul');
}
...
test('precompile yulp to yul', t => {


  yulpToYul(`object "SimpleStore" {}`, t);
  yulpToYul(``, t);
  yulpToYul(`object "SimpleStore" {
    code {
    }
  }`, t);
  yulpToYul(`object "SimpleStore" {
  ...
  t.equal(print(compile(` object "SimpleStore" { code { const nick := verbatim_1i_1o("600202", 1) } } `).results),
    `object \"SimpleStore\" { code { let nick := verbatim_1i_1o(hex"600202", 1) } }`, 'verbatim 1 2 mul');

  t.equal(print(compile(` object "SimpleStore" { code { const nick := verbatim_0i_1o("c1") } } `).results),
    `object \"SimpleStore\" { code { let nick := verbatim_0i_1o(hex"c1") } }`, 'verbatim basefee');
